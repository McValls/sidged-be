...
public class StudentLinkRestController {
	...
	@Autowired
	public StudentLinkRestController(StudentLinkService studentLinkService) {
		this.studentLinkService = studentLinkService;
		this.studentLinkMapper = new StudentLinkMapper();
	}

	@GetMapping("/")
	public List<StudentLinkDTO> getLinks() {
		Collection<StudentLink> studentLinks = this.studentLinkService.findAll();
		return studentLinks.stream().map(studentLink -> studentLinkMapper.map(studentLink))
